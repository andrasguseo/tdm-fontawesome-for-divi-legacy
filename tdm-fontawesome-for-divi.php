 ...
 */
function tdm_add_editor_styles() {
    add_editor_style( plugin_dir_url( __FILE__ ) . 'css/font-awesome.min.css' );
}
add_action( 'admin_init', 'tdm_add_editor_styles' );
...
function tdm_fontawesome_embed_code_callback() {
	$options = get_option( 'tdm_fontawesome_display_options' );
	echo 'https://use.fontawesome.com/<input type="text" id="fontawesome_embed_code" name="tdm_fontawesome_display_options[fontawesome_embed_code]" value="' . ( $options != "" ? $options[ 'fontawesome_embed_code' ] : "" ) . '" placeholder="Font Awesome Embed Code" />.js   (You can get the code <a href="https://fontawesome.com/v4.7.0/get-started/" target="_blank">here</a>.</em>)<br/>';
